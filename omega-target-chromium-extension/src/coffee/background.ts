...
chrome.runtime.onMessage.addListener(
  (
    ...
    initPromise
      .then(() => options.ready)
      .then(() => {
        // --- E2E-only helpers (safe: only accepts messages from this extension) ---
        if (typeof sender?.id === 'string' && sender.id !== chrome.runtime.id) {
          return;
        }

        if (request.method === 'e2ePing') {
          respond({ result: { ok: true } });
          return;
        }

        if (request.method === 'e2eDrawIcon') {
          const [resultColor, profileColor, size] = request.args || [];
          Promise.resolve()
            .then(async () => {
              const imageData = await offscreenManager.drawIcon(
                String(resultColor || '#000000'),
                profileColor != null ? String(profileColor) : undefined,
                Number(size || 19),
              );
              ...
              // Send only minimal info (avoid transferring large pixel buffers)
              return {
                ok: true,
                width: imageData.width,
                height: imageData.height,
                sample: Array.from(imageData.data.slice(0, 16)),
              };
            })
            .then((result) => respond({ result }))
            .catch((error: any) => respond({ error: encodeError(error) }));
          return;
        }

        if (request.method === 'e2eProxyDirect') {
          Promise.resolve()
            .then(
              () =>
            ...
            .then((value) => respond({ result: value }))
            .catch((error: any) => respond({ error: encodeError(error) }));
          return;
        }
