...
const PacGenerator = {
  /**
   * Convert non-ASCII characters to ASCII escape sequences.
   */
  ...
  /**
   * Compress the AST using UglifyJS.
   */
  compress(ast: U2.AST_Node): U2.AST_Node {
  ...
  /**
   * Generate a PAC script from a profile.
   */
  script(options: PacGeneratorOptions, profile: string | Profile, args?: PacGeneratorArgs): U2.AST_Node {
    ...
    const profileProperties: U2.AST_ObjectKeyVal[] = [];
    for (const [key, name] of Object.entries(refs)) {
      ...
      if (!p && args?.profileNotFound) {
        p = Profiles.profileNotFound(name, args.profileNotFound);
      }
      if (p) {
        profileProperties.push(new U2.AST_ObjectKeyVal({
          key,
